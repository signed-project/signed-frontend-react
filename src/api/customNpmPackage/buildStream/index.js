...
const getLatestSource = (arr) => {
  ...
  const isAlreadyExist = (address) => {
    ...
    uniqueArr.map((uArr) => {
      if (
        uArr.length > 1 &&
        uArr.filter((src) => (src.address = address)).length > 1
      ) {
        isExist = true;
      }
      return uArr;
  ...
  arr.map((source) => {
    if (
      arr.filter((s) => source.address === s.address).length > 1 &&
      !isAlreadyExist(source.address)
    ) {
      samesArr = [];
      arr
        .filter((s) => s.address === source.address)
        .map((src) => {
          samesArr.push(src);
          return src;
        });
...
export const buildStream = ({ arrPosts, arrSources }) => {
  ...
  const postSignatureValid = getDataSignatureValid({
    arr: postHashValid,
    isPost: true,
  });
  const sourcesSignatureValid = getDataSignatureValid({
    arr: sourcesHashValid,
    isPost: false,
  });
