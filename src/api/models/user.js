...
export class User {
    ...
    get newUser() {
        const date = this.data.source.updatedAt ? this.data.source.updatedAt : new Date().getTime();
        const source = {
            ...this.data.source,
            updatedAt: date,
        };

        const signatures = this.data.source.signatures ? this.data.source.signatures : getSignatures({ data: source, wif: this.data.wif });
        const hash = this.data.source.hash ? this.data.source.hash : getHash({ data: source });
