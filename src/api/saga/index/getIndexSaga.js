...
import { ACTIONS as CURRENT_STREAM_ACTIONS } from "../../storage/currentStream";
        ...
        await Promise.allSettled(
          sbs.hosts.map(async (hst) => {
            ...
            if (res?.data?.index?.recentPosts) {
              const indexPosts = Array.isArray(res?.data?.index?.recentPosts)
                ? res?.data?.index?.recentPosts
                : [];
...
function* callSelfOnTimer() {
  ...
  const { posts: tempPosts } = yield select((state) => state.currentStream);
  let { allReceivedNumber, currentAlreadySetNumber } = yield select(
    (state) => state.source
  );
  ...
  yield put({
    type: POST_ACTIONS.SET_HASHED_TARGET_POST,
    payload: index.hashedTargetPost,
  });
  yield put({
...
function getPostsByRangeTime({ loadedStream, currentTime }) {
  ...
  loadedStream.archives.map((archive) => {
    if (currTime >= archive.startDate && currTime <= archive.endDate) {
      archives.push(archive);
      if (archive.recentPosts) {
        tempArrOfPosts.push(...archive.recentPosts);
      }
    }
  ...
  if (
    currTime >= loadedStream.recentPosts.startDate &&
    currTime <= loadedStream.recentPosts.endDate
  ) {
    tempArrOfPosts.push(...loadedStream.recentPosts.posts);
  }

  console.log("loadedStream.archives");
  console.dir(loadedStream.archives);
  ...
  if (archives.length) {
    (async () => {
      for await (const archive of archives) {
        // ${hostApi.API_HOST_ASSETS}
        const jsonOfArchive = axios.get(
          `http://localhost:8010/proxy/${archive.hash.slice(
            0,
            2
          )}/${archive.hash.slice(2, 4)}/${archive.hash.slice(
            4,
            archive.hash.length
          )}.json`
        );
        console.log("JSON");
        console.dir(jsonOfArchive);
        // host api + / / hash-directories + rest hash + json extension
        // json-parse json file - get object & work
      }
    })();
  }
...
function* workerGetIndex(action) {
  const { isRegistered, currentTime } = action.payload;
  let user = yield select((state) => state.user);
  const axios = yield select((state) => state.axios.axios);
  const loadedStream = yield select((state) => state.loadedStream);

  const currPosts = getPostsByRangeTime({ loadedStream, currentTime });

  yield put({
    type: CURRENT_STREAM_ACTIONS.SET_CURRENT_STREAM,
    payload: currPosts,
  });
